...
# 匯入上下車站點資料
```{r}
ALL <- fread("E:/brain/解壓縮data/資料處理/交通站點資料/Kriging格點/北北基桃公車站群(添加鄉政市區&發展程度與Kriging天氣格點)3.csv",encoding="UTF-8")
ALL <- ALL%>%
  dplyr::select(StopUID,StopName,City,StationUID,Latitude,Longitude,
                StationGroupID,development_level,weather_grid_id)
...
### 讀取資料
```{r}
col_for_read <- c("Authority","HolderType","TicketType","SubTicketType","BoardingStopUID","BoardingStopName","BoardingStopSequence","BoardingTime","DeboardingStopUID","DeboardingStopName","DeboardingStopSequence","DeboardingTime","TransferCode","IsAbnormal","ErrorCode","Result")
```
...
```{r}
  ...
  NTWbus2024df <- process_bus_data(NTWbus2024fst,col_for_read,"2024")
  TAObus2024df <- process_bus_data(TAObus2024fst,col_for_read,"2024")
  KEEbus2024df <- process_bus_data(KEEbus2024fst,col_for_read,"2024")
...
rm(TAObus2024df, KEEbus2024df, NWTbus2024df)
gc()
```
...
rm(TPCbus2023df, TAObus2023df, KEEbus2023df)
gc()
