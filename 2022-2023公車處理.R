...
# for處理臺北公車
update_stop_codes_in_chunks <- function(df_input, df_output_prefix, verbose = TRUE, chunk_dates = NULL) {
  ...
  if (is.null(chunk_dates) || length(chunk_dates) == 0) {
    chunks_to_process_data <- list(df_full)
    chunks_to_process_labels <- list("full_data")
    ...
    chunks_to_process_data <- list()
    chunks_to_process_labels <- list() 
    
    for (i in seq_along(chunk_dates)) {
      month_range <- chunk_dates[[i]]
      month_label <- paste0("month_", paste(month_range, collapse = "_"))
      
      if (verbose) cat("正在處理月份範圍：", paste(month_range, collapse = ", "), "\n")
      chunk_df <- df_full %>% filter(DMonth %in% month_range)
      
      chunks_to_process_data[[i]] <- chunk_df
      chunks_to_process_labels[[i]] <- month_label
    }
    rm(df_full)
    gc()
  }
  ...
  for (i in seq_along(chunks_to_process_data)) {
    chunk_df <- chunks_to_process_data[[i]]
    chunk_label <- chunks_to_process_labels[[i]] 
    
    if (nrow(chunk_df) == 0) {
      if (verbose) cat(sprintf("第 %d 個資料塊 (%s) 為空，跳過。\n", i, chunk_label))
      next
    }
    if (verbose) cat(sprintf("開始處理第 %d 個資料塊 (%s) (筆數: %d)。\n", i, chunk_label, nrow(chunk_df)))
    ...
    if (verbose) {
      cat(sprintf("第 %d 個資料塊 (%s) 刪除了 %d 筆資料。\n", i, chunk_label, n_before_chunk - n_after_chunk))
    }
    
    output_filename <- paste0(df_output_prefix, "_", chunk_label, ".fst")
    if (verbose) cat(sprintf("寫入處理後的第 %d 個資料塊 (%s) 到：%s\n", i, chunk_label, output_filename))
    write_fst(chunk_df, output_filename)
    all_output_files <- c(all_output_files, output_filename) 
    
    rm(chunk_df) 
    gc()
    if (verbose) cat(sprintf("第 %d 個資料塊 (%s) 處理完成。\n", i, chunk_label))
  }
...
#2023
{
  ...
  NTPbus2023df <- process_stop_codes("E:/brain/解壓縮data/資料處理/2023/公車處理/新北市公車(遺失值填補).fst",
                                     "E:/brain/解壓縮data/資料處理/2023/公車處理/新北市公車(遺失值刪除及相同站名站碼)2.fst", drop_na_rows = TRUE, verbose = TRUE) 
  TYCbus2023df <- process_stop_codes("E:/brain/解壓縮data/資料處理/2023/公車處理/桃園市公車(遺失值填補).fst",
                                     "E:/brain/解壓縮data/資料處理/2023/公車處理/桃園市公車(遺失值刪除及相同站名站碼)2.fst", drop_na_rows = TRUE, verbose = TRUE) 
  KLCbus2023df <- process_stop_codes("E:/brain/解壓縮data/資料處理/2023/公車處理/基隆市公車(遺失值填補).fst",
                                     "E:/brain/解壓縮data/資料處理/2023/公車處理/基隆市公車(遺失值刪除及相同站名站碼)2.fst", drop_na_rows = TRUE, verbose = TRUE) 
  TPCbus2023df <- process_stop_codes("E:/brain/解壓縮data/資料處理/2023/公車處理/臺北市公車(遺失值填補).fst", 
                                     "E:/brain/解壓縮data/資料處理/2023/公車處理/臺北市公車(遺失值刪除及相同站名站碼)2.fst", drop_na_rows = TRUE, verbose = TRUE) 
